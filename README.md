...
Five-fold stratified cross-validation is the primary generalization estimate.
The separate ModernBERT model trained on all 1,000 examples is used for the
final demonstration through `predict_genre()`, but it is not evaluated on its
own training examples as evidence of held-out accuracy.
